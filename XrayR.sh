...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/sakz/wyx2685-XrayR-scripts/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/sakz/wyx2685-XrayR-scripts/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/sakz/Linux-NetSpeed-Chikage/master/tcp.sh)
    #if [[ $? == 0 ]]; then
    #    echo ""
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/sakz/wyx2685-XrayR-scripts/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
